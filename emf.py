...
class MF(nn.Module):
    def __init__(self, user_num, item_num,  latent_dim, args_config):
        super(MF, self).__init__()
        self.net= MFNet(user_num,item_num,latent_dim).to(device)
    ...
    def get_weights(self, users, infos):
        ...
        # Freeze parameters of the network except for user_embedding
        for name, param in net.named_parameters():
            if 'user' not in name:
                param.requires_grad = False

        original_weights = net.user_embedding.weight.data.clone()
        ...
        for i in range(info_num):
            ...
            up_weights = net.user_embedding.weight.data.clone()
            net.user_embedding.weight.data.copy_(original_weights)
            ...
            down_weights = net.user_embedding.weight.data.clone()
            net.user_embedding.weight.data.copy_(original_weights)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--epoch", type=int, default=10, help="number of training epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dim", type=int, default=20, help="dim_size"
    )
    parser.add_argument(
